...
import { openCredentialStore, isValidName } from "./credentials";
...
export async function runCredentialsCommand(subcommand: string, argv: string[]): Promise<number> {
...
async function setUrl(args: ParsedArgs): Promise<number> {
  ...
  if (args.flags["from-stdin"] || !process.stdin.isTTY) {
    ...
    value = await promptSecret({ prompt: `Discord webhook URL for '${name}': ` });
  }
  ...
  if (!/^https:\/\/(?:[a-z0-9-]+\.)?discord\.com\/api\/webhooks\//i.test(value)) {
    console.error("That doesn't look like a Discord webhook URL (expected https://discord.com/api/webhooks/...).");
    return 1;
  }
  ...
  if (!store.secure) {
    console.warn("Note: this backend stores the URL in plaintext on disk. Install secret-tool (Linux) or use macOS/Windows for OS-native encryption.");
  }
  return 0;
}
...
async function getUrl(args: ParsedArgs): Promise<number> {
  ...
  console.log(`'${name}' is set in ${store.description}. Re-run with --reveal to print the URL.`);
  return 0;
}
...
async function clearUrl(args: ParsedArgs): Promise<number> {
  ...
  if (args.flags.all) {
    ...
    console.log(`Cleared ${names.length} stored URL${names.length === 1 ? "" : "s"}.`);
    return 0;
  }
