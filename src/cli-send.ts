...
import { sendDiscordWebhook, type DiscordMessage } from "./discord";
...
async function resolveWebhookUrl(args: ParsedArgs): Promise<string | undefined> {
  ...
  if (name) {
    ...
    console.error(`No stored URL for '${name}'. Run 'fledge discord set-url --name ${name}' first.`);
    process.exit(1);
  }
...
async function buildMessage(args: ParsedArgs): Promise<DiscordMessage> {
  ...
  if (args.options.json !== undefined) {
    const raw = (args.options.json as string) === "-" ? await readStdin() : (args.options.json as string);
...
function parseFields(raw: unknown): Array<{ name: string; value: string; inline?: boolean }> | undefined {
  const list = raw === undefined ? [] : Array.isArray(raw) ? (raw as string[]) : [raw as string];
...
function resolveTimestamp(raw: unknown, now: boolean | undefined): string | undefined {
