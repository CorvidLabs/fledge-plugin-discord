...
import { CredentialError, SERVICE_NAME, type CredentialStore } from "./types";
...
export class SecretToolStore implements CredentialStore {
  ...
  async set(name: string, value: string): Promise<void> {
    ...
    if (result.code !== 0) {
      throw new CredentialError(`secret-tool write failed (exit ${result.code}). ${result.stderr.trim()}`);
    }
    await this.index.add(name);
  }

  async get(name: string): Promise<string | undefined> {
    const result = await spawnCmd(["secret-tool", "lookup", "service", SERVICE_NAME, "account", name]);
    if (result.code !== 0 || result.stdout.length === 0) return undefined;
    return result.stdout.replace(/\r?\n$/, "");
  }

  async delete(name: string): Promise<boolean> {
    const result = await spawnCmd(["secret-tool", "clear", "service", SERVICE_NAME, "account", name]);
