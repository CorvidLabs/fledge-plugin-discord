...
export class DPAPIStore implements CredentialStore {
  ...
  async set(name: string, value: string): Promise<void> {
    ...
    const result = await spawnCmd(
      ["powershell", "-NoProfile", "-NonInteractive", "-Command", ENCRYPT_SCRIPT],
      value,
    );
    if (result.code !== 0) {
      throw new CredentialError(`DPAPI encryption failed (exit ${result.code}). ${result.stderr.trim()}`);
    }
  ...
  async get(name: string): Promise<string | undefined> {
    ...
    const result = await spawnCmd(
      ["powershell", "-NoProfile", "-NonInteractive", "-Command", DECRYPT_SCRIPT],
      blob,
    );
