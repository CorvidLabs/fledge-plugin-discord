...
export class KeychainStore implements CredentialStore {
  ...
  async set(name: string, value: string): Promise<void> {
    await this.spawnSilent(["security", "delete-generic-password", "-s", SERVICE_NAME, "-a", name]);
    ...
    if (result.code !== 0) {
      throw new CredentialError(`Keychain write failed (exit ${result.code}). ${redact(result.stderr)}`);
    }
